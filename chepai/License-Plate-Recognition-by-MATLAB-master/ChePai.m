...
imwrite(CuDingWei, 'CuDingWei.jpg');
...
imwrite(ChePaiLvBo, 'ChePaiLvBo.jpeg');
[p,q] = size(ChePaiLvBo);
XX = zeros(1,q);
for j=1:q
    for i=1:p
...
while (i<q)
    ...
    part = ChePaiLvBo(:,l:r);
    count = count + 1;
    imwrite(part, ['part',num2str(count),'.jpg']);
    while ((XX(1,i)==0)&&(i<q))
        i = i + 1;
    end
    l = i;
